...
const OurShop = () => {
    ...
    return (
        <div>
            <SharedCover bimg={cover} title={'OUR SHOP'} subTitle={'Would you like to try a dish?'} />

            <section className='max-w-5xl mx-auto p-4 md:p-6 lg:p-8'>
                <ul className='flex flex-wrap gap-2 justify-center cursor-pointer my-6 font-semibold uppercase text-base md:text-xl lg:text-2xl'>
            ...
            {/* Tab Info Section */}
            <section className='my-8 lg:my-16 max-w-6xl mx-auto p-4'>
                <div className={tabs === 1 ? "block" : "hidden"}>
                    <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4'>
                        {salad?.map((data, idx) => <FoodCard key={idx} data={data} />)}
                    </div>
                </div>
                <div className={tabs === 2 ? "block" : "hidden"}>
                    <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4'>
                        {pizza?.map((data, idx) => <FoodCard key={idx} data={data} />)}
                    </div>
                </div>
                <div className={tabs === 3 ? "block" : "hidden"}>
                    <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4'>
                        {soup?.map((data, idx) => <FoodCard key={idx} data={data} />)}
                    </div>
                </div>
                <div className={tabs === 4 ? "block" : "hidden"}>
                    <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4'>
                        {dessert?.map((data, idx) => <FoodCard key={idx} data={data} />)}
                    </div>
                </div>
                <div className={tabs === 5 ? "block" : "hidden"}>
                    <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4'>
                        {drinks?.map((data, idx) => <FoodCard key={idx} data={data} />)}
                    </div>
                </div>
            </section>
        </div>
